...
> If you're a fresh agent session in this repo, read this top-to-bottom before doing anything. This is the universal agent-context source of truth; `CLAUDE.md` is a compatibility symlink for Claude Code. The architecture has gone through several pivots; the current shape is locked in by explicit decisions, and reverting to earlier patterns will cause friction.
...
- `CLAUDE.local.md` — your private Claude Code addendum with live state, account routing, and brand notes (gitignored, auto-loaded by Claude Code alongside `CLAUDE.md`). Sample shape: `CLAUDE.local.md.example`.
...
- `skills/<name>/SKILL.md` — universal skill source of truth; `.claude/skills` is a symlink to this directory so Claude Code can invoke the same verbs
...
1. **Tab-aware**: skills do tab discovery via `bash scripts/switch_to_platform_tab.sh <substring> <fallback-url>` (which uses `scripts/find_platform_tab.sh` internally). **Never use `agent-browser tab list`** — it auto-spawns a fresh Chrome on CDP attach failure even when the existing Chrome is HTTP-healthy, which kills the user's session. **And never hunt for a tab by looping `agent-browser tab 0..N`** — each `tab <n>` activates (raises) a tab, so a sweep yanks Chrome to the foreground on every step (it stole the user's window focus repeatedly on 2026-05-20). The helper switches by stable Chrome target id; if it can't land, skip the platform (warm) or abort it (post) — do not improvise a sweep. Skills never close pinned platform tabs.
   - **Spawned-tab cleanup**: cron wrappers snapshot the tab set before invoking the skill and prune anything new at the end:
     ```bash
     TAB_BASELINE="${HOME}/.social-skills/state/tab-baseline-<name>-$$.json"
     bash scripts/tab_baseline_save.sh "$TAB_BASELINE"
     # ... claude --print "/skill" ...
     bash scripts/close_spawned_tabs.sh "$TAB_BASELINE"
     rm -f "$TAB_BASELINE"
     ```
     The closer no-ops when `keep_tabs_open: true` is set in `config/brand.json`. Platform tabs (any URL in `essential_tabs`) and browser-internal URLs (`about:*`, `chrome://*`, `devtools://*`, `chrome-extension://*`, `view-source:*`) are NEVER closed regardless of baseline state. Default is `false` (= close spawned tabs). Ad-hoc skill invocations that bypass the cron wrappers can call the same two helpers around their work to get the same cleanup.
...
5. **Pre-pad iPhone media before posting**:
   - **Images** → `bash scripts/pad_ios_screenshot.sh <input> [output] [mode]` pads to 4:5 (Instagram feed). Plain modes: `edge` (seamless), `blur` (Apple-style), `random` (palette), `#RRGGBB`. Decorative / themed modes (random gradient bg + SVG decoration layer): `gradient`, `bloom`, `sparkle`, `cosmic`, `divine` (golden crosses + scripture words), `holy` (divine + Bibles + flying-dove silhouettes), `lovely` (hearts), `mother`, `father`, `dream` (faded words over blur), `ascii-art` (terminal-green monospace glyph field), `shadow`, `lament`, `easter`, `christmas`, `pentecost`. `surprise` rolls one of the lighter fancy modes. The decoration layer is rasterized via `rsvg-convert`, not `magick` — magick's SVG path can't resolve fontconfig text fonts on macOS so any `<text>` element makes it error with "unable to read font ''". `brew install librsvg` if it's missing.
   - **ASCII treatments** → Daily devotionals can choose between `ascii-art` (static 4:5 image using `/ascii-art` visual vocabulary: terminal glyphs, figlet/banner energy, boxed/text-art feel) and `ascii-video` (animated 9:16 MP4 using `/ascii-video` visual vocabulary: layered character fields, cinematic motion). Both render through local scripts so cron remains deterministic; reserve them for scripture-as-text / app-native posts or explicit user requests.
...
For brand-specific composite skills (daily-content cron, brand-tooling helpers), see `personal.example/` — copy templates into `skills/post-daily-<your-brand>/` (gitignored; surfaced to Claude through `.claude/skills`) and customize. See `PERSONAL.md`.
...
- **NEVER discover a tab by looping `agent-browser tab 0..N` + `get url`.** Unlike the curl-based finder, every `agent-browser tab <n>` *activates* (raises) that tab, so a sweep drags Chrome to the foreground on each iteration — exactly what made the 2026-05-20 13:43 warm run repeatedly steal the user's window focus. The agent improvised the sweep because the index-based switch had landed on the wrong tab (Chrome reorders `/json/list` by recency, so a page-array index computed after another tab was activated points at the wrong tab). `switch_to_platform_tab.sh` now switches by **stable Chrome target id** (`curl /json/activate/<id>`, which never drifts) with a bounded index fallback and no sweep. If the helper still can't verify the switch, **skip the platform (warm) or abort it (post)** — never sweep.
...
- Refactored committed skills out of Claude-specific `.claude/skills/` into universal `skills/`; `.claude/skills` is now a compatibility symlink to `../skills`.
- Refactored committed agent context out of Claude-specific `CLAUDE.md` into universal `AGENTS.md`; `CLAUDE.md` is now a compatibility symlink to `AGENTS.md`.
